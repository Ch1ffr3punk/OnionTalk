...
var (
	...
	previousMessageWasSingleLine = true
)
...
func receiveMessages(conn net.Conn, gcm cipher.AEAD, wg *sync.WaitGroup, isServer bool) {
	...
	for {
		...
		if strings.Contains(message, "\n") {
			if previousMessageWasSingleLine {
				fmt.Println()
			}
			fmt.Printf("%s\n", message)
			previousMessageWasSingleLine = false
		...
		if showPrompts && !inMultiLineMode && waitingForInput {
			fmt.Printf(">")
			waitingForInput = false
		}
	}
...
func sendMessages(conn net.Conn, gcm cipher.AEAD, wg *sync.WaitGroup, isServer bool) {
	...
	for {
		...
		if input != "" {
			...
			if showPrompts && !inMultiLineMode {
				fmt.Printf(">")
			}
		}
	}
